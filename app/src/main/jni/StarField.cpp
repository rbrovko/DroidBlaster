...
StarField::StarField(android_app *pApplication, TimeManager &pTimeManager,
        ...
        mVertexBuffer(), mTexture(-1), mShaderProgram(0),
        aPosition(-1), uProjection(-1),
        uHeight(-1), uTime(-1), uTexture(-1) {
...
status StarField::load() {
    ...
    mVertexBuffer = mGraphicsManager.loadVertexBuffer((uint8_t *) vertexBuffer, mStarCount *
            sizeof(Vertex));
    delete[] vertexBuffer;
    if (mVertexBuffer == 0) {
        goto ERROR;
